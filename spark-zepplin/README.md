...
| `Master.Image`          | Container image name               | `kodelint/spark-for-helm`                                 |
| `Master.ImageTag`       | Container image tag                | `v0.1`                                                      |
...
| `Worker.Image`               | Container image name                 | `kodelint/spark-for-helm`                                 |
| `Worker.ImageTag`            | Container image tag                  | `v0.1`                                                      |
| `Worker.Replicas`            | k8s hpa and deployment replicas      | `3`                                                        |
| `Worker.ReplicasMax`         | k8s hpa max replicas                 | `20`                                                       |
...
| `Worker.Autoscaling`         | Enable horizontal pod autoscaling    | `true`                                                    |
...
| `Zeppelin.Image`               | Container image name             | `kodelint/zepplin-for-helm`                                        |
| `Zeppelin.ImageTag`            | Container image tag              | `v0.1`                                                     |
...
1. Using sparks inbuilt support for K8s clusters. This is mostly suited for jobs that don't require any user interaction or monitoring. See https://spark.apache.org/docs/latest/running-on-kubernetes.html#submitting-applications-to-kubernetes. To sumbit a job, typically you would run `kubectl proxy` to setup a proxy to the cluster at `localhost:8001` usually. The you would run call spark-submit. For example, to run the SparkPi sample app https://spark.apache.org/examples.html, you would run ```bin/spark-submit \
    ...
    --conf spark.kubernetes.container.image=kodelint/spark-2.4.0 \
...
To upgrade to this chart from 0.2.2 you will have to update the docker images to point the images in `values.yaml`. 